...
class MainWindow(QOpenGLWindow) :
  ...
  def initializeGL(self) :
    ...
    self.buildVAO();
    VAOFactory.listCreators()


  def buildVAO(self) :
    verts=[Vec3(0.0,1.0,1.0),
          Vec3(0.0,0.0,-1.0),
          Vec3(-0.5,0.0,1.0),
          Vec3(0.0,1.0,1.0),
          Vec3(0.0,0.0,-1.0),
    ...
    verts.append(n);
    verts.append(n);
    # pyngl only accepts raw floats (as we cant cast easily so convert to float array)
    data=[]
    for v in verts :
      data.append(v.m_x)
      data.append(v.m_y)
    ...
    self.vao.bind()
    #self.vao.setData(VertexData(sys.getsizeof(data),data,GL_STATIC_DRAW))
    self.vao.setData(data)
    self.vao.setVertexAttributePointer(0,3,GL_FLOAT,0,0)
    self.vao.setVertexAttributePointer(2,3,GL_FLOAT,0,12*3)
